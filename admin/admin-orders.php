...
<body>
	...
	<?php 
		...
		$order_result = pg_query($pgcon,"SELECT * FROM tbl_order_cancel_msg WHERE status = 0");
		$ord_count = pg_num_rows($order_result);
	
		if ($ord_count > 0) {
			?>
				<div style="width: 10%; margin-left: 695px;" class="subscribe_form">
					<a href="order-cancel-msg.php"><button type="submit" class="submit" name="submit">Notification
					<span style="background-color: slateblue;" class="badge bg-primary notify"><?php echo $ord_count ?></span>
					</button></a>
				</div>
			<?php
		}
	...
	<table class="content-table">
		...
		<tbody>
			...
			<?php 
				...
				//execute the query for tbl_order 
				$result = pg_query($pgcon,"SELECT * FROM tbl_orders ORDER BY order_id DESC LIMIT 20");
				$sr = 1;
				...
				while ($rows = pg_fetch_assoc($result)) {
					...
					$qty = $rows['qty'];
					$size = $rows['size'];
					$shipping_charges = $rows['shipping_charges'];
					$total = $rows['total'];
					$status = $rows['status'];
					...
					while ($menu_rows = pg_fetch_assoc($menu_result)) {
					...
					?>
						...
						<tr class="active-row">
							...
							<td><?php echo "<img src='admin-menu/menu-img/" . $menu_img . " ' alt='user' width='100'>"; ?></td>
							<td><?php echo $menu_name ?></td>
							<td><?php echo $qty ?></td>
							<td><?php echo $size ?></td>
							<td><?php echo $status ?></td>
							<td><?php echo $total ?></td>
							<td><a href="update-order.php?order_id=<?php echo $order_id ?>">Update</a>
									...
									<div class="submit">
										<a href="update-category.php?category_id=<?php 
										// echo $category_id; ?>">Update</a>
									</div> -->
							</td>
							<td>
								<a href="delete-order.php?order_id=<?php echo $order_id ?>">Delete</a>
							</td>
						</tr>
					<?php
				}
